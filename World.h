...
public:
	...
	Box* Create(float x, float y, float width, float height, bool isStatic, const std::string& name) {
...
#pragma endregion
	...
	int StepSimulation(std::vector<Collision*>& colls) {
		int collCount = 0;
		auto it = Boxes.begin();
		while (it != Boxes.end()) {
			Box* box = it->second;
