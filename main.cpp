...
LRESULT CALLBACK WinProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INPUT:
	{
		...
		{
			raw.ProcessMouseMessage(&pRI->data.mouse);
		}

		// Process the Keyboard Messages
		if (pRI->header.dwType == RIM_TYPEKEYBOARD)
		{
			raw.ProcessKeyboardMessage(&pRI->data.keyboard);
		}
	...
	case WM_MOUSEMOVE:
		raw.mouseX = LOWORD(lParam);
		raw.mouseY = HIWORD(lParam);
		break;

		//case WM_KEYDOWN:
		//	raw.lastKeyState[wParam] = raw.currentKeyState[wParam];
		//	raw.currentKeyState[wParam] = 1;
		//	break;
		//case WM_KEYUP:
		//	raw.lastKeyState[wParam] = raw.currentKeyState[wParam];
		//	raw.currentKeyState[wParam] = 0;
		//	break;
